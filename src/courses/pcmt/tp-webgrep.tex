...
\begin{description}
\item[Code de départ associé :] \url{https://github.com/DistributedComputing/TP-webgrep}.
\end{description}
...
\begin{lstlisting}
  $ java webgrep.WebGrep Nantes https://fr.wikipedia.org/wiki/Nantes
...
\section{Algorithme à implémenter}
...
Une grande portion du code a déjà été faite pour vous.
Téléchargez le code de départ dans le dépôt \href{https://github.com/DistributedComputing/TP-webgrep}{GitHub} du TP.
...
\item[\texttt{Tools.initialize(args)}] analyse les arguments du programme pour extraire l'expression régulière, les URL de départ et les options d'affichage.  
  La liste complète des options peut être affichée en lançant le programme avec l'argument \texttt{"--help"}.  
...
\paragraph{Remarque} le pare-feu des salles de TP bloque l'accès à Internet pour les requêtes automatiques.
L'option \lstinline{-O} (ou \lstinline{--offline}), activée par défaut lorsque le programme est lancé sans argument,
simule le chargement des pages sans se connecter à internet.

\pagebreak
\section{Architecture parallèle}
