...
\begin{exercice}[Horloges logiques de Lamport]
  On considère l'exécution suivante, dont les pas sont
  soit l'émission ou la réception d'un message, représentés par les flèches,
  soit des actions internes, représentées par $\bullet$.
  ...
  \begin{questions}
  \item Donner la restriction de la relation \emph{happens before} aux événements d'émission des messages.
  \item Donner la date de Lamport associée à chacun des événements de l'exécution.
  \item Comparer les messages selon les dates de Lamport de leurs événements d'émission. Comparer cet ordre à celui obtenu à la question~1.
  \item Donner un ordre total sur les messages compatible avec leurs dates de Lamport.
  \end{questions}
